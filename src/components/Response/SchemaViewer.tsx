...
export function SchemaViewer() {
  ...
  if (result.state === "invalid-schema") {
    return (
      <div className="flex items-center justify-center h-full">
        <div className="flex flex-col items-center gap-3 text-center px-8">
          <svg className="h-10 w-10 text-red-500/40" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <span className="text-sm text-red-500">Invalid JSON Schema</span>
          <span className="text-[10px] text-muted-foreground/60">The schema could not be parsed. Check for syntax errors.</span>
        </div>
      </div>
    );
  }
  ...
  if (result.state === "no-response") {
    return (
      <div className="flex items-center justify-center h-full text-muted-foreground text-sm">
        <span>Send a request to validate against the schema</span>
      </div>
    );
  }
  ...
  if (result.state === "not-json") {
    return (
      <div className="flex items-center justify-center h-full">
        <div className="flex flex-col items-center gap-3 text-center px-8">
          <svg className="h-10 w-10 text-yellow-500/40" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 9v2m0 4h.01" />
          </svg>
          <span className="text-sm text-yellow-500">Response is not JSON</span>
  ...
  return (
    <div className="h-full flex flex-col">
      {/* Summary header */}
      <div className="shrink-0 flex items-center gap-2 px-4 py-2.5 border-b bg-destructive/5">
      ...
      <div className="flex-1 overflow-auto p-3 space-y-2">
        {Object.entries(grouped).map(([path, pathErrors]) => (
          ...
          >
            <div className="flex items-center gap-1.5 px-3 py-1.5 bg-secondary/30 border-b text-xs font-mono text-muted-foreground">
              <svg className="h-3 w-3 text-destructive shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            {pathErrors.map((err, i) => (
              <div
                key={i}
                className="px-3 py-1.5 border-b last:border-0 hover:bg-accent/20 transition-colors text-xs flex items-start gap-2"
              >
